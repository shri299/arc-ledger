package io.arcledger.service.impl;

import io.arcledger.domain.SyntheticQuestion;
import io.arcledger.repository.SyntheticQuestionRepository;
import io.arcledger.service.*;
import jakarta.annotation.PostConstruct;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.boot.autoconfigure.condition.ConditionalOnProperty;
import org.springframework.stereotype.Service;

import java.time.Instant;
import java.time.temporal.ChronoUnit;
import java.util.*;
import java.util.concurrent.ConcurrentHashMap;

@Service
@ConditionalOnProperty(name = "arcledger.retrieval.vector-store-provider", havingValue = "in-memory")
public class InMemoryNarrativeVectorStore implements NarrativeVectorStore {
    private final SyntheticQuestionRepository repository;
    private final EmbeddingService embeddingService;
    private final Map<UUID, double[]> vectors = new ConcurrentHashMap<>();
    private final double semanticWeight, entityWeight, versionWeight, recencyWeight;

    public InMemoryNarrativeVectorStore(SyntheticQuestionRepository repository, EmbeddingService embeddingService,
        @Value("${arcledger.retrieval.semantic-weight:0.65}") double semanticWeight,
        @Value("${arcledger.retrieval.entity-weight:0.15}") double entityWeight,
        @Value("${arcledger.retrieval.version-weight:0.10}") double versionWeight,
        @Value("${arcledger.retrieval.recency-weight:0.10}") double recencyWeight) {
        this.repository = repository;
        this.embeddingService = embeddingService;
        this.semanticWeight = semanticWeight;
        this.entityWeight = entityWeight;
        this.versionWeight = versionWeight;
        this.recencyWeight = recencyWeight;
    }

    @PostConstruct
    public void rebuildIndex() {
        repository.findAll().forEach(question ->
            vectors.put(question.getId(), embeddingService.embed(question.getQuestion())));
    }

    @Override public void index(SyntheticQuestion question, double[] embedding) {
        vectors.put(question.getId(), Arrays.copyOf(embedding, embedding.length));
    }

    @Override
    public List<RetrievalHit> search(UUID storyId, String query, UUID entityId, int limit) {
        double[] queryVector = embeddingService.embed(query);
        String normalizedQuery = query.toLowerCase(Locale.ROOT);
        return repository.findByStoryId(storyId).stream()
            .filter(question -> entityId == null || question.getEntity().getId().equals(entityId))
            .filter(question -> vectors.containsKey(question.getId()))
            .map(question -> hit(question, queryVector, normalizedQuery))
            .sorted(Comparator.comparingDouble(RetrievalHit::score).reversed())
            .limit(Math.max(1, limit)).toList();
    }

    private RetrievalHit hit(SyntheticQuestion question, double[] queryVector, String normalizedQuery) {
        double semantic = cosine(queryVector, vectors.get(question.getId()));
        double entity = normalizedQuery.contains(question.getEntity().getName().toLowerCase(Locale.ROOT)) ? 1.0 : 0.0;
        double validity = question.isCurrent() ? 1.0 : 0.0;
        long ageDays = Math.max(0, ChronoUnit.DAYS.between(question.getCreatedAt(), Instant.now()));
        double recency = 1.0 / (1.0 + ageDays / 30.0);
        double score = semanticWeight * semantic + entityWeight * entity + versionWeight * validity + recencyWeight * recency;
        if (!question.isCurrent()) score -= 0.20;
        return new RetrievalHit(question.getId(), question.getEntity().getId(), question.getStateVersion().getId(),
            question.getScene().getId(), question.getEntity().getName(), question.getQuestion(), question.getAnswer(),
            question.isCurrent(), question.getStateVersion().getVersion(), score);
    }

    private double cosine(double[] left, double[] right) {
        double dot = 0, a = 0, b = 0;
        for (int i = 0; i < Math.min(left.length, right.length); i++) {
            dot += left[i] * right[i]; a += left[i] * left[i]; b += right[i] * right[i];
        }
        return a == 0 || b == 0 ? 0 : dot / (Math.sqrt(a) * Math.sqrt(b));
    }
}
