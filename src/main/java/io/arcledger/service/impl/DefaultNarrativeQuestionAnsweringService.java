...
public class DefaultNarrativeQuestionAnsweringService implements NarrativeQuestionAnsweringService {
    ...
    public DefaultNarrativeQuestionAnsweringService(NarrativeRetrievalService retrievalService,
    ...
    public Answer answer(UUID storyId, String query) {
        ...
        String fallback = grounded.stream().map(RetrievalHit::answer).distinct().reduce((a, b) -> a + " " + b).orElseThrow();
        String answer = inferenceService.composeGroundedAnswer(query, grounded).orElse(fallback);
