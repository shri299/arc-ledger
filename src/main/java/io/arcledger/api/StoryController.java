...
public class StoryController {
    ...
    public StoryResponse create(@Valid @RequestBody CreateStoryRequest request) {
        ...
        return new StoryResponse(story.getId(), story.getTitle(), story.getDescription(), story.getCreatedAt(), StoryRole.OWNER);
    }
    @GetMapping
    public PageResponse<StoryResponse> list(@RequestParam(defaultValue = "0") @Min(0) int page,
        ...
        return ApiModels.page(service.listForCurrentUser(page, size), access ->
            new StoryResponse(access.story().getId(), access.story().getTitle(), access.story().getDescription(),
                access.story().getCreatedAt(), access.role()));
    }
    ...
    public PageResponse<ChapterResponse> chapters(@PathVariable UUID storyId,
                                                   @RequestParam(defaultValue = "0") @Min(0) int page,
                                                   @RequestParam(defaultValue = "100") @Min(1) @Max(100) int size) {
        return ApiModels.page(service.listChaptersForCurrentUser(storyId, page, size), chapter ->
            new ChapterResponse(chapter.getId(), storyId, chapter.getNumber(), chapter.getTitle(), chapter.getCreatedAt()));
    }
}
